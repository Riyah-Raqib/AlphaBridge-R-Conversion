...
FEATURE_MATRIX <- R6Class("FEATURE_MATRIX",
  public = list(
    ...
    get_feature_matrix_dict = function(pae, plddt, plddt_matrix, pae_plddt, symmetric_pae, contact_matrix, confidence_matrix, masked_confidence_matrix, masked_contact_matrix) {
      matrix_dict <- list()
      matrix_dict[["pae"]] <- pae
    ...
    print_matrix_dict = function(matrix_dict) {
      excluded_keys <- c("pae","plddt","symmetric_pae","pae_plddt","masked_confidence_matrix", "masked_contact_matrix")
...
extract_sequence_info = function() {
  ...
  return(list(list_sequence_info, chain_tuple, structure_sequence_list, polymer_chain_dict))
    ...
    extract_matrix_dict = function() {
      ...
      masked_confidence_matrix[mask_lower] <- NA  # Set masked entries to NA
      
      matrix_dict <- self$get_feature_matrix_dict(pae, plddt, plddt_matrix, pae_plddt, symmetric_pae, contact_matrix, confidence_matrix, masked_confidence_matrix, masked_contact_matrix)
